...
#
# Detectors currently present: **Fine-tuned M1** (≤30 dB) and **M2** (all dB), scored
# by the canonical `eval_detector_masks.py`. The deployed **Coherent Power** and
# **Zero-shot DINOv3** are pending a container run on the LTE captures — this notebook
# **auto-includes them** on every plot as soon as their masks are added under
# `notebooks/sweep_lte/` and the tables are regenerated (see the last cell).
...
ALL_ORDER = ["coherent_power", "cuda_dino", "finetuned_dino", "finetuned_dino_m2"]
...
DET_SHORT = {"coherent_power": "Coherent", "cuda_dino": "Zero-shot",
             "finetuned_dino": "M1", "finetuned_dino_m2": "M2"}  # compact printouts
